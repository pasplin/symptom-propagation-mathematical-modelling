...
timestep
%}
function [Classes] = ODE_SEIR_model_isb_int(para,ICs,maxtime)
...
dS = -sum(FOI)*S;
dV = -(1-para.eta)*sum(FOI)*V;

dE(1) = ((1-(1-para.alpha)*(1-para.nu(1)))*FOI(1) + para.nu(1)*(1-para.alpha)*FOI(2))*S + (1-para.eta)*sum(FOI)*V - para.epsilon*E(1);
dE(2) = ((1-(1-para.alpha)*(1-para.nu(2)))*FOI(2) + para.nu(2)*(1-para.alpha)*FOI(1))*S  - para.epsilon*E(2);
