%{ 
ODE_SEIR_model_ib_int(para,ICs,maxtime)
...
timestep
%}
function [Classes] = ODE_SEIR_model_ib_int(para,ICs,maxtime)
...
for s = 1:m
    dE(s) = ((1-(1-para.alpha)*(1-para.nu(s)))*FOI(s) + para.nu(s)*(1-para.alpha)*(sum(FOI)-FOI(s)))*(S + (1-para.eta)*V) - para.epsilon*E(s);
