...
timestep
%}
function [Classes] = ODE_SEIR_model_sb_int(para,ICs,maxtime)
...
dE(1) = para.eta*V*sum(FOI) + ((1-(1-para.alpha)*(1-para.nu(1)))*FOI(1) + para.nu(1)*(1-para.alpha)*FOI(2))*((1-para.eta)*V + S) - para.epsilon*E(1);
dE(2) = ((1-(1-para.alpha)*(1-para.nu(2)))*FOI(2) + para.nu(2)*(1-para.alpha)*FOI(1))*(S + (1-para.eta)*V)  - para.epsilon*E(2);
